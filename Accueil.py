"""
B23 — Dossier de Vol — Page d'accueil
...
le menu de gauche pour naviguer dans les sections du dossier.
...
icao_help = f"Aérodromes pré-remplis dans la base : {', '.join(ICAO_LIST)}"

c1, c2 = st.columns(2)
with c1:
    st.text_input("Élève / Pilote", key="eleve", placeholder="Prénom Nom")
...
with c2:
    ...
    st.text_input(
        ...
        key="depart", max_chars=4,
        help=icao_help,
    )
    st.text_input(
        "Aérodrome d'arrivée (OACI)",
        key="arrivee", max_chars=4,
        help=icao_help,
    )
    st.text_input(
        "Dégagement(s) — OACI séparés par virgule",
        key="degagements",
        help="Ex : LFPT, LFFE, LFOX",
    )

# Récap aéroports reconnus
infos = []
for label, icao in [("Départ", st.session_state.depart),
                    ("Arrivée", st.session_state.arrivee)]:
    ad = get_airport(icao)
    if ad:
        infos.append(f"**{label}** : {icao} — {ad['name']} · "
                     f"alt {ad['elevation_ft']} ft · "
                     f"pistes {', '.join(r['ident'] for r in ad['runways'])}")
if infos:
    st.info("\n\n".join(infos))
...
st.info("💡 Toutes vos saisies sont conservées entre les sections (et sauvegardables "
        "en JSON via la sidebar gauche).")
